...
Token经常被误当成知识、价值或跨模型通用的能力单位，需要先排除这几种理解：一个时代用什么单位计量生产，产能、价格和账单就以什么口径被看见。
...
“预测下一个Token”听起来像手机输入法，于是有人把大模型贬为“高级自动补全”。这也不准确。规模化训练会让模型在参数中形成可迁移的表示和模式，使它能够翻译、概括、编程、分类并处理训练中没有逐字出现的新任务。能力的外观不能改变它的生成机制：模型首先生成在上下文中合适的后续，而不是先去现实世界核验每一句话。

同一种生成机制带来了两种结果：模型能够组合大量语言模式，写出人没有预先编好的回答；也可能把真实姓名、专业语气和虚构事实组合成一段极具说服力的文字。流畅可以直接来自生成过程，真实却需要数据、检索、工具和验证程序共同保证。

但一枚Token为什么能从简单编号变成翻译、代码和分析？答案不在生成的那一刻，而在模型建成以前经历的漫长训练。
